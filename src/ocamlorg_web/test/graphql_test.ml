...
let package_result_test name version expect () =
  ...
  let result =
    match package with
    | Error _ -> "Not Found"
    | Ok package -> package |> Package.name |> Package.Name.to_string
  in
...
let package_versions_result_test name from upto total_packages () =
  ...
  let num_of_packages_returned =
    ...
    | Error _ -> 0
    | Ok package_versions -> List.length package_versions.packages
  in
  Alcotest.(check int)
...
let () =
  Alcotest.run "ocamlorg"
    [
      ( "get_info_test",
        [
          Alcotest.test_case "returns the number of dependencies found" `Quick
            get_info_test;
        ] );
      ( "is_in_range_test_within_range",
        [
          Alcotest.test_case "returns true" `Quick
            (is_in_range_test
      ...
      ( "is_in_range_test_outside_range",
        [
          Alcotest.test_case "returns false" `Quick
            (is_in_range_test
               (Package.Version.of_string "3.12.0")
               None
      ...
      ( "is_valid_params_test_valid_params",
        [
          Alcotest.test_case "returns valid_params" `Quick
            (is_valid_params_test (List.length packages) 0 "valid_params");
        ] );
      ( "is_valid_params_test_wrong_limit",
        [
          Alcotest.test_case "returns wrong_limit" `Quick
      ...
      ( "package_result_test_with_invalid_name",
        [
          Alcotest.test_case "returns error message" `Quick
            (package_result_test "ocdfggaml" None "Not Found");
        ] );
      ( "package_result_test_with_valid_name_and_version",
        [
          Alcotest.test_case "returns the package name" `Quick
